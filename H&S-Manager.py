...
class GreenfieldGame:
    ...
    def initialize_card_data(self):
        return {
            ...
            "Curse of the Drained Brain": {"desc": "Choose three questions in different categories. The seekers cannot ask those questions for the rest of your run.", "cost_str": "Discard you hand", "cost_type": "hand_all"},
            ...
            "Curse of the Hide-And-Seek-Ception": {"desc": "All seekers but one must close their eyes. Without discussion, the remaining seeker must go somewhere at least 300 meters away and not within direct eyeshot in any direction. The other seekers must find them, without asking them for any information, before asking another question.", "cost_str": "The seekers must be off-transit, at least 300 meters from a transit station.", "cost_type": "special"},
            ...
            "Curse of the Mind Meld": {"desc": "When you are trying to complete this curse, you are able to use creative mode, but you are not able to leave the bedrock block and destroy other things. First, go outdoors, change to creative mode and place a bedrock block. Without any discussion, on the count of three, any two seekers place any block on the bedrock at the same time. Assuming they’re not the same word, they must wait 30 seconds, and on top of the previous block, both seekers place another block that they believe to be the midpoint between the last two words. You may not place any blocks that have already been placed, or do anything at all to indicate what block you will place next. Seekers may not ask another question until they both say the same word. After completing this curse, you must destroy the block of tower, change to adventure mode, and use /clear to clear your inventory.", "cost_str": "Discard a card", "cost_type": "any", "cost_val": 1},
            ...
            "Discard Me": {"desc": "You can choose to discard this card to pay for the entirety of a curse's casting cost, so long as that card's casting cost involves discarding cards, but not the whole hand.", "cost_str": "None", "cost_type": "none"},
    ...
    def update_loop(self):
        if self.game_phase == "HIDING":
            ...
            self.timer_win.update_display("SEEKERS RELEASED!", f"{m:02d}:{s:02d}", "#ff4500")
            self.local_status.config(text=f"SURVIVAL TIMER: {m:02d}:{s:02d}")
    ...
    def process_effect(self, card_name, data):
        # FIX: Curse of The Open Mind sets max hand size to 999
    ...
    def confirm_end(self):
        if messagebox.askyesno("END GAME", "Confirm and calculate final survival time?"):
            ...
            em, es = divmod(int(self.elapsed_time), 60)
            bm, bs = divmod(bonus_mins * 60, 60)
            fm, fs = divmod(int(final_time), 60)
            results = (f"--- GAME OVER ---\n\n"
                      f"Original Hiding Time: {em}m {es}s\n"
                      f"Total Bonus Time: {bm}m {bs}s\n"
                      f"Percentage Multiplier: +{int(mult*100)}%\n\n"
                      f"FINAL SURVIVAL TIME: {fm}m {fs}s")
